...
var ipInfoCacheMutex sync.Mutex

const ipInfoAPIURL = "http://ipinfo.io/%s/json"
...
func getIPInfo(host string) (*IPInfo, error) {
	ipInfoCacheMutex.Lock()
	defer ipInfoCacheMutex.Unlock()

	if ipInfo, exists := ipInfoCache[host]; exists {
		return ipInfo, nil
	}

	if isValidIP(host) {
		resp, err := (&http.Client{Timeout: 5 * time.Second}).Get(fmt.Sprintf(ipInfoAPIURL, host))
		if err != nil {
			return nil, err
		}
		...
		ipInfoCache[host] = &ipInfo
		return &ipInfo, nil
	} else {
		return &IPInfo{Org: "Unknown"}, nil
	}
}
